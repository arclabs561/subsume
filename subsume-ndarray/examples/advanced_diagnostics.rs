...
use subsume_core::{
    training::{
        calibration::{
            adaptive_calibration_error, expected_calibration_error, reliability_diagram,
        },
        diagnostics::{GradientFlowAnalysis, TrainingStats},
        quality::{kl_divergence, ContainmentHierarchy, IntersectionTopology, VolumeDistribution},
    },
    Box,
};
...
fn main() -> Result<(), subsume_core::BoxError> {
    ...
    for step in 0..15 {
        ...
        if step % 5 == 0 {
            if let Some(ratio) = gradient_flow.check_imbalance(2.0) {
                println!(
                    "   Step {}: Gradient imbalance detected! Ratio: {:.2}",
                    step, ratio
                );
            }
            let sparsity = gradient_flow.gradient_sparsity(0.01);
            println!(
                "   Step {}: Gradient sparsity: {:.2}%",
                step,
                sparsity * 100.0
            );
        }
    ...
    println!(
        "     Mean: {:.6}, Std Dev: {:.6}",
        vol_dist.mean, vol_dist.std_dev
    );
    println!(
        "     CV: {:.4}, Entropy: {:.4}",
        vol_dist.cv, vol_dist.entropy
    );
    println!(
        "     Quantiles: Q25={:.4}, Q50={:.4}, Q75={:.4}, Q95={:.4}",
        vol_dist.quantiles.0, vol_dist.quantiles.1, vol_dist.quantiles.2, vol_dist.quantiles.3
    );
    ...
    let kl = kl_divergence(
        learned_volumes.iter().copied(),
        target_volumes.iter().copied(),
    );
    ...
    for i in 0..diagram.bin_centers.len() {
        println!(
            "     Bin {}: predicted={:.3}, actual={:.3}, count={}",
            i, diagram.bin_centers[i], diagram.empirical_accuracies[i], diagram.bin_counts[i]
        );
    }
    println!();
    ...
    if let Some(sibling_ratio) = topology.sibling_intersection_ratio {
        println!(
            "     Sibling intersection ratio: {:.4} (should be low)",
            sibling_ratio
        );
    }
    if let Some(pc_ratio) = topology.parent_child_intersection_ratio {
        println!(
            "     Parent-child intersection ratio: {:.4} (should be high)",
            pc_ratio
        );
    }
    println!();
    ...
    println!(
        "     Volume distribution: entropy={:.4}, has_hierarchy={}",
