...
use subsume_core::{
    ...
    training::{
        diagnostics::{
            TrainingStats, DepthStratifiedGradientFlow,
            PhaseDetector,
        },
        quality::{
            VolumeConservation,
            DimensionalityUtilization, GeneralizationMetrics,
        },
    },
};
...
fn main() -> Result<(), subsume_core::BoxError> {
    ...
    if let Some((min_depth, max_depth, ratio)) = depth_flow.check_depth_imbalance(2.0) {
        ...
        println!("     Min depth {}: {:.4}, Max depth {}: {:.4}",
                 min_depth, means[&min_depth],
                 max_depth, means[&max_depth]);
        println!("     Ratio: {:.2}x (higher levels getting more gradients)", ratio);
    }
    println!();
    ...
    for (loss, grad, label) in phases {
        ...
        println!("   {} training: loss={:.2}, grad={:.2} -> Phase: {:?}",
    ...
    println!("   Children volumes: Mammal={:.4}, Bird={:.4}",
             mammal_vol, bird_vol);
    println!("   Sum of children: {:.4}", mammal_vol + bird_vol);
    
    let mean_ratio = conservation.mean_ratio();
    println!("   Mean parent-child ratio: {:.4} (should be <= 1.0)", mean_ratio);
    println!("   Violation rate: {:.2}%", conservation.violation_rate() * 100.0);
    
    if let Some((min, max, mean, median)) = conservation.ratio_statistics() {
        println!("   Ratio statistics: min={:.4}, max={:.4}, mean={:.4}, median={:.4}",
                 min, max, mean, median);
    }
    println!();
    ...
    for i in 0..5 {
        ...
        util.record_box(
            ...
            vec![offset + 2.0, 0.1 + i as f32 * 0.05, offset * 0.8 + 1.6, offset * 1.2 + 2.4].into_iter(),
        );
    }
    
    let effective_dim = util.effective_dimensionality(1.0);
    println!("   Effective dimensionality (threshold=1.0): {}", effective_dim);
    ...
    for epoch in 0..30 {
        ...
        if epoch % 10 == 0 {
            ...
            if let Some((min_d, max_d, ratio)) = depth_flow.check_depth_imbalance(2.0) {
                println!("     ⚠️  Depth gradient imbalance: {:.2}x (depth {} vs {})",
                         ratio, max_d, min_d);
            }
            ...
            if violation_rate > 0.0 {
                println!("     ⚠️  Volume conservation violations: {:.1}%",
