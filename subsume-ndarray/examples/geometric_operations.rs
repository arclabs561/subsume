...
fn main() -> Result<(), subsume_core::BoxError> {
    ...
    println!("  Result: [{:.1},{:.1}]→[{:.1},{:.1}]", 
             union_ab.min()[0], union_ab.min()[1],
             union_ab.max()[0], union_ab.max()[1]);
    println!("  Volume: {:.3}\n", union_ab.volume(1.0)?);
    ...
    for (i, box_) in [&box_a, &box_b, &box_c].iter().enumerate() {
        ...
        println!("Box {} center: [{:.2}, {:.2}]", 
                 ['A', 'B', 'C'][i], center[0], center[1]);
    }
    println!();
    ...
    println!("Distance A→B: {:.3} (overlapping: {})", dist_ab, dist_ab == 0.0);
    ...
    println!("Boxes overlapping with query [0.3,0.3]→[0.7,0.7]: {:?}", overlapping);
    println!();
    ...
    for i in 0..collection.len() {
        ...
        println!("  Box {}: {:.3} {}", i, containment, if containment > 0.99 { "✓" } else { "✗" });
    }
