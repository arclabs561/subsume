...
use subsume_core::{
    ...
    temperature_scheduler, volume_containment_loss, volume_overlap_loss,
    MIN_TEMPERATURE,
};
...
fn main() -> Result<(), subsume_core::BoxError> {
    ...
    println!("   High-dimensional box (20D, side=0.1): volume = {:.6e}", volume);
    ...
    println!("   Box volume: {:.2}, threshold: [0.01, 5.0], lambda: 0.1", vol);
    ...
    let decay_rate = 0.95;
    println!("   Initial temperature: {}, decay rate: {}", initial_temp, decay_rate);
    ...
    println!("     Containment prob: {:.4}, Loss: {:.4}", containment_prob, loss_pos);
    ...
    println!("     Containment prob: {:.4}, Loss: {:.4}", containment_prob_neg, loss_neg);
    ...
    for step in 0..5 {
        ...
        println!("   Step {}: temp={:.4}, vol={:.4}, reg={:.4}",
