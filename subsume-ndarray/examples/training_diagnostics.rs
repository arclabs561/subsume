...
use subsume_core::training::{
    diagnostics::{LossComponents, TrainingStats},
    metrics::{hits_at_k, mean_rank, mean_reciprocal_rank, ndcg},
};
...
fn main() -> Result<(), subsume_core::BoxError> {
    ...
    println!(
        "     Containment: {:.2}, Regularization: {:.2}, Constraint: {:.2}",
        imbalanced.containment_loss, imbalanced.regularization_loss, imbalanced.constraint_loss
    );
    ...
    println!(
        "     Containment: {:.2}, Regularization: {:.2}, Constraint: {:.2}",
        balanced.containment_loss, balanced.regularization_loss, balanced.constraint_loss
    );
