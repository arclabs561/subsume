...
mod tests {
    ...
    fn test_dasgupta_2020_gradient_density_quantitative() {
        ...
        let hard_box_a = NdarrayBox::new(
            array![0.0, 0.0],
            array![1.0, 1.0],
            1.0,
        ).unwrap();
        
        let hard_box_b = NdarrayBox::new(
            array![2.0, 2.0],
            array![3.0, 3.0],
            1.0,
        ).unwrap();
        
        let gumbel_box_a = NdarrayGumbelBox::new(
            array![0.0, 0.0],
            array![1.0, 1.0],
            1.0,
        ).unwrap();
        
        let gumbel_box_b = NdarrayGumbelBox::new(
            array![2.0, 2.0],
            array![3.0, 3.0],
            1.0,
        ).unwrap();
        ...
        assert_eq!(hard_prob, 0.0, "Hard boxes should have zero probability for disjoint boxes");
        
        // Gumbel boxes: non-zero probability (non-zero gradients)
        let gumbel_prob = gumbel_box_a.containment_prob(&gumbel_box_b, 1.0).unwrap();
        assert!(gumbel_prob > 0.0, "Gumbel boxes should have non-zero probability for disjoint boxes, got {}", gumbel_prob);
        assert!(gumbel_prob < 0.1, "Gumbel probability should be small for disjoint boxes, got {}", gumbel_prob);
    ...
    fn test_regd_2025_crowding_effect_mitigation() {
        ...
        let euclidean_mean = euclidean_distances.iter().sum::<f32>() / euclidean_distances.len() as f32;
        let euclidean_variance = euclidean_distances.iter()
            .map(|d| (d - euclidean_mean).powi(2))
            .sum::<f32>() / euclidean_distances.len() as f32;
        ...
        let depth_variance = depth_distances.iter()
            .map(|d| (d - depth_mean).powi(2))
            .sum::<f32>() / depth_distances.len() as f32;
    ...
    fn test_concept2box_2023_vector_outside_box_formula() {
        ...
        assert!((dist - expected).abs() < 1e-5,
            "Distance {} should be approximately {}", dist, expected);
        ...
        assert!((dist_partial - 1.0).abs() < 1e-5,
            "Partial distance {} should be 1.0", dist_partial);
    }
    ...
    fn test_depth_distance_zero_volume_edge_case() {
        ...
        assert!(dist.is_ok(), "Depth distance should handle zero-volume boxes");
    }
    ...
    fn test_boundary_distance_identical_boxes() {
        ...
        assert_eq!(dist_val, 0.0, "Boundary distance for identical boxes should be 0");
    }
}
