...
mod matrix_e2e_tests {
    ...
    fn create_hierarchy() -> BoxCollection<NdarrayBox> {
        ...
        collection.push(NdarrayBox::new(
            ...
            array![0.3, 0.3, 0.3],
            1.0,
        ).unwrap()); // Subcategory A1
        
        collection.push(NdarrayBox::new(
            array![0.2, 0.0, 0.0],
            array![0.5, 0.3, 0.3],
            1.0,
        ).unwrap()); // Subcategory A2
        
        collection.push(NdarrayBox::new(
            array![0.5, 0.0, 0.0],
            array![0.7, 0.3, 0.3],
            1.0,
        ).unwrap()); // Subcategory B1
    ...
    fn test_containment_matrix_hierarchy() {
        ...
        for i in 0..8 {
            assert!((matrix[i][i] - 1.0).abs() < 1e-5, "Diagonal element {i} should be 1.0");
        }
        ...
        assert!(matrix[1][3] > 0.8, "Category A should contain Subcategory A1");
        assert!(matrix[1][4] > 0.8, "Category A should contain Subcategory A2");
        ...
        assert!(matrix[1][2] < 0.5, "Category A should not contain Category B");
    }
    ...
    fn test_containing_boxes_hierarchy() {
        ...
        assert!(containing.contains(&3), "Subcategory A1 should contain leaf");
        assert!(!containing.contains(&2), "Category B should not contain leaf from Category A");
        ...
        assert!(containing_subcat.contains(&0), "Root should contain subcategory");
        assert!(containing_subcat.contains(&1), "Category A should contain subcategory");
    ...
    fn test_contained_boxes_hierarchy() {
        ...
        assert!(contained_in_a.contains(&3), "Category A should contain Subcategory A1");
        assert!(contained_in_a.contains(&4), "Category A should contain Subcategory A2");
        assert!(contained_in_a.contains(&6), "Category A should contain Leaf A1a");
        assert!(contained_in_a.contains(&7), "Category A should contain Leaf A1b");
        assert!(!contained_in_a.contains(&2), "Category A should not contain Category B");
    }
    ...
    fn test_matrix_with_gumbel_boxes() {
        ...
        collection.push(NdarrayGumbelBox::new(
            array![0.0, 0.0],
            array![1.0, 1.0],
            1.0,
        ).unwrap());
    ...
    fn test_matrix_disjoint_boxes() {
        ...
        assert!(matrix[0][1] < 0.5, "Disjoint boxes should have low containment");
        assert!(matrix[1][0] < 0.5, "Disjoint boxes should have low containment");
    }
}
