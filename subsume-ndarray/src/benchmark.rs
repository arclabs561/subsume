...
impl BenchmarkSuite {
    ...
    fn benchmark_optimizers(
        ...
        for run in 0..config.num_runs {
            ...
            let metrics = run_single_benchmark_with_config(&mut optimizer, "Adam", &config.eval_config)?;
            all_metrics_adam.push(metrics);
            println!("    Run {}: MRR = {:.4}", run + 1, all_metrics_adam.last().unwrap().final_mrr);
        }
        ...
        self.results.push(BenchmarkResult {
            ...
            hyperparams: {
                ...
                h.insert("learning_rate".to_string(), config.eval_config.learning_rate);
                h
            },
            metrics: avg_adam,
        ...
        for run in 0..config.num_runs {
            let mut optimizer = AdamW::new(config.eval_config.learning_rate, config.eval_config.weight_decay);
            let metrics = run_single_benchmark_with_config(&mut optimizer, "AdamW", &config.eval_config)?;
            all_metrics_adamw.push(metrics);
            println!("    Run {}: MRR = {:.4}", run + 1, all_metrics_adamw.last().unwrap().final_mrr);
        }
        ...
        for run in 0..config.num_runs {
            ...
            let metrics = run_single_benchmark_with_config(&mut optimizer, "SGD", &config.eval_config)?;
            all_metrics_sgd.push(metrics);
            println!("    Run {}: MRR = {:.4}", run + 1, all_metrics_sgd.last().unwrap().final_mrr);
        }
    ...
    fn save_results(&self, output_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for result in &self.results {
            ...
            result_json.insert("final_mrr".to_string(), serde_json::json!(result.metrics.final_mrr));
            result_json.insert("final_hits_at_1".to_string(), serde_json::json!(result.metrics.final_hits_at_1));
            result_json.insert("final_hits_at_10".to_string(), serde_json::json!(result.metrics.final_hits_at_10));
            result_json.insert("total_training_time".to_string(), serde_json::json!(result.metrics.total_training_time));
            result_json.insert("hyperparams".to_string(), serde_json::json!(result.hyperparams));
            result_json.insert("loss_history".to_string(), serde_json::json!(result.metrics.loss_history));
            result_json.insert("validation_mrr".to_string(), serde_json::json!(result.metrics.validation_mrr));
            result_json.insert("epoch_times".to_string(), serde_json::json!(result.metrics.epoch_times));

            json_results.insert(result.config_name.clone(), serde_json::Value::Object(result_json));
        }
    ...
    /// Generate comparison plots.
    fn generate_comparison_plots(&self, output_path: &Path) -> Result<(), std::boxed::Box<dyn std::error::Error>> {
        #[cfg(feature = "plotting")]
        {
            ...
            let lr_results: Vec<_> = self.results.iter()
    ...
    for _epoch in 0..config.epochs {
        ...
        for batch in train_triples.chunks(config.batch_size) {
            for triple in batch {
                ...
                let (pos_score, tail_min, tail_max, head_key) = if let (Some(head_box), Some(tail_box)) = (
...
fn average_metrics(metrics_list: &[EvaluationMetrics]) -> EvaluationMetrics {
    ...
    if !metrics_list[0].loss_history.is_empty() {
        ...
        avg.loss_history = (0..len)
            .map(|i| {
                metrics_list.iter().map(|m| m.loss_history.get(i).copied().unwrap_or(0.0)).sum::<f32>() / n
            })
            .collect();
    }
    ...
    if !metrics_list[0].validation_mrr.is_empty() {
        ...
        avg.validation_mrr = (0..len)
            .map(|i| {
                metrics_list.iter().map(|m| m.validation_mrr.get(i).copied().unwrap_or(0.0)).sum::<f32>() / n
            })
            .collect();
    }
    ...
    if !metrics_list[0].epoch_times.is_empty() {
        ...
        avg.epoch_times = (0..len)
            .map(|i| {
                metrics_list.iter().map(|m| m.epoch_times.get(i).copied().unwrap_or(0.0)).sum::<f64>() / n as f64
            })
            .collect();
    }
    ...
    avg.total_training_time = metrics_list.iter().map(|m| m.total_training_time).sum::<f64>() / n as f64;

    avg
}

/// Compute standard deviation of metrics.
fn compute_std_dev(metrics_list: &[EvaluationMetrics], avg: &EvaluationMetrics) -> EvaluationMetrics {
