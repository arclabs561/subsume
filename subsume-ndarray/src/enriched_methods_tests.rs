...
mod enriched_methods_tests {
    ...
    #[test]
    fn test_center() {
        let box_ = NdarrayBox::new(array![0.0, 2.0, 4.0], array![2.0, 4.0, 6.0], 1.0).unwrap();
    ...
    fn test_distance_partially_overlapping() {
        ...
        let box_b = NdarrayBox::new(array![1.5, 0.0], array![2.5, 1.0], 1.0).unwrap();
    ...
    fn test_overlap_matrix() {
        ...
        collection.push(NdarrayBox::new(array![2.0, 2.0], array![3.0, 3.0], 1.0).unwrap());
    ...
    fn test_overlapping_boxes() {
        ...
        let query = NdarrayBox::new(array![0.2, 0.2], array![0.8, 0.8], 1.0).unwrap();

        // Check actual overlap probabilities
        let overlap_0 = collection
            .get(0)
            .unwrap()
            .overlap_prob(&query, 1.0)
            .unwrap();
        let overlap_1 = collection
            .get(1)
            .unwrap()
            .overlap_prob(&query, 1.0)
            .unwrap();
        let overlap_2 = collection
            .get(2)
            .unwrap()
            .overlap_prob(&query, 1.0)
            .unwrap();
        ...
        let overlapping = collection
            .overlapping_boxes(&query, threshold, 1.0)
            .unwrap();

        // Query overlaps with box 0 (contained) and box 1 (partial overlap)
        assert!(
            overlapping.contains(&0),
            "Query should overlap with box 0 (overlap={})",
            overlap_0
        );
        assert!(
            overlapping.contains(&1),
            "Query should overlap with box 1 (overlap={})",
            overlap_1
        );
        assert!(
            !overlapping.contains(&2),
            "Query should not overlap with box 2 (overlap={})",
            overlap_2
        );
    }
    ...
    fn test_nearest_boxes() {
        ...
        collection.push(NdarrayBox::new(array![0.0, 0.0], array![1.0, 1.0], 1.0).unwrap());
        collection.push(NdarrayBox::new(array![2.0, 2.0], array![3.0, 3.0], 1.0).unwrap());
        collection.push(NdarrayBox::new(array![5.0, 5.0], array![6.0, 6.0], 1.0).unwrap());

        let query = NdarrayBox::new(array![0.5, 0.5], array![1.5, 1.5], 1.0).unwrap();
    ...
    fn test_bounding_box() {
        ...
        collection.push(NdarrayBox::new(array![0.0, 0.0], array![1.0, 1.0], 1.0).unwrap());
        collection.push(NdarrayBox::new(array![2.0, 2.0], array![3.0, 3.0], 1.0).unwrap());
        collection.push(NdarrayBox::new(array![1.5, 0.5], array![2.5, 1.5], 1.0).unwrap());
    ...
    fn test_union_gumbel() {
        ...
        let box_b = NdarrayGumbelBox::new(array![0.5, 0.5], array![1.5, 1.5], 0.5).unwrap();
    ...
    fn test_center_gumbel() {
        let box_ = NdarrayGumbelBox::new(array![1.0, 3.0], array![3.0, 5.0], 1.0).unwrap();
    ...
    fn test_distance_gumbel() {
        ...
        let box_b = NdarrayGumbelBox::new(array![2.0, 2.0], array![3.0, 3.0], 0.5).unwrap();
