...
mod edge_case_tests {
    ...
    use serde_json;
    use subsume_core::{Box, BoxCollection, BoxEmbedding, BoxError, GumbelBox};
    ...
    fn test_collection_out_of_bounds() {
        let collection: BoxCollection<NdarrayBox> =
            vec![NdarrayBox::new(array![0.0], array![1.0], 1.0).unwrap()].into();
    ...
    fn test_containment_matrix_single_box() {
        let collection: BoxCollection<NdarrayBox> =
            vec![NdarrayBox::new(array![0.0, 0.0], array![1.0, 1.0], 1.0).unwrap()].into();
    ...
    fn test_very_small_volume() {
        let small_box = NdarrayBox::new(array![0.0, 0.0], array![1e-6, 1e-6], 1.0).unwrap();
