...
This transformation is logically equivalent to the original query but enables low-dimensional embedding: each conjunctive query is embedded as a box in $d$-dimensional space, then the distance to the EPFO query is computed as the minimum distance to any of the $N$ conjunctive query boxes: $dist_"agg"(v; q) = min({dist_"box"(v; q^((1))), ..., dist_"box"(v; q^((N)))$)$, taking the minimum distance to any box. This strategy avoids the theoretical requirement of dimension $Theta(|V|)$ for embedding EPFO queries as single points or boxes.
