...
where $\beta \gt 0$ is the scale parameter (constant across dimensions) and $\mu_x, \mu_y$ are learnable location parameters.
...
**Step 1:** Substitute $u = (x-\mu_x)/\beta$ and $v = (y-\mu_y)/\beta$, so $dx = \beta du$ and $dy = \beta dv$. The region $y \gt x$ becomes $v \gt u - \delta$ where $\delta = (\mu_y - \mu_x)/\beta$.
