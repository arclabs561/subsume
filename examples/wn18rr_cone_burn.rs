//! Batched **relation-aware** cone (ConE) embedding training on WN18RR using the
//! Burn backend.
//!
//! Run (CPU / rayon multi-core):
//!   cargo run --features burn-ndarray --example wn18rr_cone_burn --release
//!
//! Run (Metal / GPU on Apple Silicon):
//!   cargo run --features burn-wgpu --example wn18rr_cone_burn --release
//!
//! For a quick smoke on CPU, cap the run:
//!   TRAIN_LIMIT=8000 EPOCHS=15 DIM=32 BATCH=512 NEG=5 \
//!     cargo run --features burn-ndarray --example wn18rr_cone_burn --release
//!
//! Environment variables (all optional):
//!   DIM, EPOCHS, LR, MARGIN, NEG, BATCH, ADV_TEMP, K, REG, TRAIN_LIMIT,
//!   REPORT_EVERY
//!
//! Cone embeddings score containment by ConE angular distance (lower distance =
//! better containment) after per-relation axis translations. The loss is
//! distance-contrastive (no InfoNCE path), so `INFONCE` is not read here.
...
use subsume::trainer::burn_cone_trainer::BurnConeTrainer;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=== WN18RR relation-aware Cone Embedding Training (Burn) ===\n");
    ...
    println!(
        "\nConfig: dim={dim}, epochs={epochs}, lr={lr}, margin={margin}, \
         ...
         train_limit={train_limit}, report_every={report_every}"
    );
    ...
    let mut trainer = BurnConeTrainer::<Backend>::new();
