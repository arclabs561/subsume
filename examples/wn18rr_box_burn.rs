//! Batched **relation-aware** hard-box embedding training on WN18RR using the
//! Burn backend.
//!
//! Run (CPU / rayon multi-core):
//!   cargo run --features burn-ndarray --example wn18rr_box_burn --release
//!
//! Run (Metal / GPU on Apple Silicon):
//!   cargo run --features burn-wgpu --example wn18rr_box_burn --release
//!
...
//!
//! Box embeddings score containment `P(tail ⊆ head)` after per-relation
//! translations; WN18RR is a general link-prediction benchmark, so containment
//! geometry is competitive on the hierarchical relations (`_hypernym`,
//! `_member_meronym`) and weaker on the symmetric ones.
...
use subsume::trainer::burn_box_trainer::BurnBoxTrainer;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=== WN18RR relation-aware Box Embedding Training (Burn) ===\n");
    ...
    let mut trainer = BurnBoxTrainer::<Backend>::new();
