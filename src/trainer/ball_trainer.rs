...
use crate::trainer::{CpuBoxTrainingConfig, NegativeSamplingStrategy};
use crate::BoxError;
...
impl BallTrainer {
    ...
    pub fn init_embeddings(
        ...
        let entities: Vec<Ball> = (0..num_entities)
            .map(|_| {
                let center: Vec<f32> = (0..dim).map(|_| self.rng.gen_range(-0.1..0.1)).collect();
                let log_radius = self.rng.gen_range(-1.0..0.0);
        ...
        let relations: Vec<BallRelation> = (0..num_relations)
            .map(|_| {
                let translation: Vec<f32> =
                    (0..dim).map(|_| self.rng.gen_range(-0.01..0.01)).collect();
                let log_scale: f32 = self.rng.gen_range(-0.1..0.1);
    ...
    pub fn train_epoch(
        ...
        for &idx in &indices {
            ...
            let neg_tail_idx = loop {
                let neg = self.rng.gen_range(0..num_entities);
                if neg != tail_idx {
                    break neg;
                }
            };
